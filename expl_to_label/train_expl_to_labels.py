...
parser.add_argument("--results_dir", type=str,
                    default='results_expl_to_labels')
...
parser.add_argument("--linear_fc", action='store_false',
...
params.save_title += "_" + params.optimizer + "_Enc" + \
    str(params.enc_rnn_dim) + "_bs" + \
    str(params.batch_size) + "_gpu" + str(params.gpu)
...
all_sentences = train['expl_1'] + snli_dev['expl_1'] + snli_dev['expl_2'] + \
...
for split in ['expl_1', 'expl_2', 'expl_3']:
    ...
    snli_test[split] = np.array([['<s>'] + [word for word in sent.split()
                                            if word in word_vec] + ['</s>'] for sent in snli_test[split]])
...
def trainepoch(epoch):
    ...
    for stidx in range(0, len(expl_1), params.batch_size):
        total_iterations = int(len(expl_1) / params.batch_size)
        print(f"epoch {epoch}: iteration {stidx/params.batch_size}/{total_iterations}")
        # prepare batch
...
def evaluate_dev(epoch):
    ...
    if eval_acc > val_acc_best:
        ...
        current_best_model_path = best_model_prefix + \
            '_devACC{0:.3f}__epoch_{1}_model.pt'.format(eval_acc, epoch)
