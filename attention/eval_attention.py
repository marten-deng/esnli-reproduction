    ...
    for i in range(0, len(s1), batch_size):
        ...
        for index in range(1, 4):
            ...
            out_expl = esnli_net((s1_batch, s1_len), (s2_batch, s2_len),
                                 input_expl_batch, mode="teacher", visualize=False)
            # ppl
        ...
        pred_expls = esnli_net((s1_batch, s1_len), (s2_batch, s2_len),
                               input_expl_batch, mode="forloop", visualize=visualize)
        if visualize:
            ...
            all_weights = torch.cat([weights_1[:, :len(
                sentence1_split)], weights_2[:, :len(sentence2_split)]], 1).transpose(1, 0)
            ...
            y = np.array(
                list(range(len(sentence1_split) + len(sentence2_split))))
            ...
            plt.savefig(os.path.join(current_run_dir, time.strftime(
                "%d:%m") + "_" + time.strftime("%H:%M:%S") + "_att_" + str(i) + ".png"), dpi=1000)
            # plt.show()
...
def eval_datasets_without_expl(esnli_net, expl_to_labels_net, which_set, data, word_vec, word_vec_expl, word_emb_dim, batch_size, print_every, current_run_dir):
    ...
    headers = ["gold_label", "Premise",
               "Hypothesis", "pred_label", "pred_expl"]
    expl_csv = os.path.join(current_run_dir, time.strftime(
    ...
    for i in range(0, len(s1), batch_size):
        ...
        pred_expls = esnli_net(
            (s1_batch, s1_len), (s2_batch, s2_len), expl_t0, mode="forloop", visualize=False)

        pred_expls_with_sos = np.array(
            [['<s>'] + [word for word in sent.split()] + ['</s>'] for sent in pred_expls])
        pred_expl_batch, pred_expl_len = get_batch(
            pred_expls_with_sos, word_vec_expl)
        pred_expl_batch = Variable(pred_expl_batch.cuda())
...
def eval_all(esnli_net, expl_to_labels_net, criterion_expl, params):
    ...
    dev_csv = os.path.join(current_run_dir, time.strftime(
        "%d:%m") + "_" + time.strftime("%H:%M:%S") + "_" + "artifacts.csv")
    remove_file(dev_csv)
    ...
    snli_train = get_train(params.esnli_path, preproc,
                           params.min_freq, params.n_train)
    ...
    snli_sentences = snli_train['s1'] + snli_train['s2'] + snli_train['expl_1'] + snli_dev['s1'] + snli_dev['s2'] + snli_dev['expl_1'] + \
        snli_dev['expl_2'] + snli_dev['expl_3'] + snli_test['s1'] + snli_test['s2'] + \
        snli_test['expl_1'] + snli_test['expl_2'] + snli_test['expl_3']
    ...
    # SNLI
    test_acc, test_bleu_score, test_ppl = evaluate_snli_final(esnli_net, expl_to_labels_net, criterion_expl, 'snli_test', snli_test,
                                                              snli_dev_no_unk, snli_test_no_unk, word_vec, word_index, batch_size, print_every, current_run_dir, visualize=True)
