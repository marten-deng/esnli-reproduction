...
class AttentionDecoder(nn.Module):
    def __init__(self, config):
        ...
        self.proj_inp_dec = nn.Linear(
            2 * self.att_hid_dim + self.word_emb_dim, self.dec_rnn_dim)
        if self.decoder_type == 'gru':
            self.decoder_rnn = nn.GRU(self.dec_rnn_dim, self.dec_rnn_dim,
                                      self.n_layers_dec, bidirectional=False, dropout=self.dpout_dec)
        else:  # 'lstm'
            self.decoder_rnn = nn.LSTM(self.dec_rnn_dim, self.dec_rnn_dim,
                                       self.n_layers_dec, bidirectional=False, dropout=self.dpout_dec)
    ...
    def forward(self, expl, enc_out_s1, enc_out_s2, s1_embed, s2_embed, mode, visualize):
        ...
        # init decoder
        if self.use_init:
            init_0 = self.context_proj(context).expand(
                self.n_layers_dec, batch_size, self.dec_rnn_dim)
        else:
        ...
        if mode == "teacher":
            for t_dec in range(current_T_dec):
                ...
                inp_att_1 = self.att_ht_proj1(
                    enc_out_s1).transpose(1, 0).transpose(2, 1)
                assert_sizes(inp_att_1, 3, [
                ...
                att_applied_1 = torch.bmm(
                    att_weights_1, self.att_ht_before_weighting_proj1(enc_out_s1).permute(1, 0, 2))
                ...
                inp_att_2 = self.att_ht_proj2(
                    enc_out_s2).transpose(1, 0).transpose(2, 1)
                assert_sizes(inp_att_2, 3, [
                             batch_size, self.att_hid_dim, self.max_T_encoder])
                ...
                att_applied_2 = torch.bmm(
                    att_weights_2, self.att_ht_before_weighting_proj2(enc_out_s2).permute(1, 0, 2))
                ...
                input_dec = torch.cat([expl[t_dec].unsqueeze(
                    0), att_applied_perm_1, att_applied_perm_2], 2)
            ...
            assert_sizes(
                out_expl, 3, [current_T_dec, batch_size, self.n_vocab])
            return out_expl
            ...
            while t_dec < self.max_T_decoder and not array_all_true(finished):
                ...
                for ii in range(batch_size):
                    assert abs(att_weights_1[ii].data.sum(
                    ) - 1) < 1e-5, str(att_weights_1[ii].data.sum())
                    assert abs(att_weights_2[ii].data.sum(
                    ) - 1) < 1e-5, str(att_weights_2[ii].data.sum())
...
class BLSTMEncoder(nn.Module):
    ...
    def forward(self, sent_tuple):
        ...
        if self.pool_type == "max":
            padding_value = -100
        sent_output_padding = nn.utils.rnn.pad_packed_sequence(
            sent_output, False, padding_value)[0]
        sent_output = nn.utils.rnn.pad_packed_sequence(
            sent_output, False, 0)[0]

        # Un-sort by length
        idx_unsort = torch.from_numpy(idx_unsort).cuda() if self.is_cuda() \
            else torch.from_numpy(idx_unsort)
        sent_output = sent_output.index_select(1, Variable(idx_unsort))
        sent_output_padding = sent_output_padding.index_select(
            1, Variable(idx_unsort))
        ...
        if sent_output.size(0) < self.max_T_encoder:
            pad_tensor = Variable(torch.zeros(
                self.max_T_encoder - sent_output.size(0), sent_output.size(1), sent_output.size(2)).cuda())
...
class eSNLIAttention(nn.Module):
    ...
    def forward(self, s1, s2, expl, mode, visualize):
        ...
        # u = max_T_enc x bs x (2 * enc_dim) ; u_emb = 1 x bs x (2 * enc_dim)
        u, u_emb = self.encoder(s1)
